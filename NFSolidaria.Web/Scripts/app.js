$(function () {
    ...
    function carregaDados() {
        var mes = $("[name='Periodo']").val();

        if (mes < 1) {
            return;
        }

        var mesNome = $("[name='Periodo'] option:selected").text();
        $.get("/Home/MostrarDados/?mes=" + mes, function (data) {
            preparaChart(data, mesNome);
        });
    }

    function preparaChart(data, mesNome) {

        $('#chartUpload').highcharts({
            chart: {
                type: 'column'
            },
            title: {
            ...
            xAxis: {
                categories: [
                   '<strong style="font-size:15px;">' + mesNome + '</strong>'
                ],
                crosshair: true
            },
            yAxis: {
            ...
            tooltip: {
                ...
                pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' +
                    '<td style="padding:0"><b>{point.y:.1f} reais</b></td></tr>',
                footerFormat: '</table>',
                shared: true,
                useHTML: true
            },
