...
{
    public class Comparer<T> : System.Collections.Generic.IComparer<T>
    {
        ...
        #region .: Constructors :.
        public Comparer(string orderBy)
        {
            Initialize(orderBy);
        }

        public Comparer(Sort[] sortProperties)
        {
        ...
        public void Initialize(Sort[] sortProperties)
        {
        ...
        protected Sort[] ParseOrderBy(string orderBy)
        {
            ...
            for (int i = 0; i < props.Length; i++)
            {
                ...
                sortProps[i] = new Sort(prop, descending);
            }

            return sortProps;
        }

        protected DynamicMethod CreateDynamicCompareMethod(Sort[] sortProperties)
        {
            DynamicMethod dm = new DynamicMethod("DynamicComparison", typeof(int), new Type[] { typeof(T), typeof(T) }, typeof(Comparer<T>));
            ...
            foreach (Sort property in sortProperties) // For each of the properties we want to check inject the following il.
            {
        ...
        protected void CheckSortProperties(Sort[] sortProperties)
        {
            if (sortProperties == null)
                sortProperties = new Sort[0];

            Type instanceType = typeof(T);
            if (!instanceType.IsPublic)
                throw new ArgumentException(string.Format("Type \"{0}\" is not public.", typeof(T).FullName));

            foreach (Sort sProp in sortProperties)
            {
